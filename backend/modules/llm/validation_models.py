...
class Role(BaseModel):
    ...
    end_date: str = Field(
        ..., description="End date (YYYY-MM, 'Present', or 'Unknown')"
    )
...
class Skill(BaseModel):
    ...
    source: str = Field(
        ...,
        description="Where the skill was found (e.g., 'Work Experience', 'Projects')",
    )
...
class ResumeAnalysisResponse(BaseModel):
    ...
    total_experience_years: int = Field(
        ge=0, le=50, default=0, description="Total years of professional experience"
    )
    roles: List[Role] = Field(
        default_factory=list, description="List of work experience roles"
    )
    work_experience_raw: str = Field(
        ..., description="1-4 sentences summarizing relevant work experience"
    )
    skills: Dict[str, Skill] = Field(
        default_factory=dict, description="Dictionary of skills with details"
    )
    projects: List[Project] = Field(
        default_factory=list, description="List of relevant projects"
    )
    leadership_signals: bool = Field(
        default=False, description="Whether leadership/ownership signals were detected"
    )
    leadership_justification: str = Field(
        default="", description="Explanation of leadership assessment"
    )
    candidate_fit_summary: str = Field(
        ..., description="2-3 lines explaining suitability vs JD"
    )
    ...
    def validate_eligibility_consistency(cls, v, values):
        ...
        if "fit_score" in values and values["fit_score"] is not None:
            ...
            if expected_eligible and v == "Not Eligible":
                ...
                print(
                    f"[VALIDATION] Correcting eligibility: fit_score {score} should be Eligible, not {v}"
                )
                return EligibilityStatus.ELIGIBLE
            elif not expected_eligible and v == "Eligible":
                # Auto-correct logical inconsistency
                print(
                    f"[VALIDATION] Correcting eligibility: fit_score {score} should be Not Eligible, not {v}"
                )
    ...
    class Config:
        ...
        json_encoders = {EligibilityStatus: lambda v: v.value}
...
def create_resume_analysis_agent(
    provider="openrouter", model="anthropic/claude-3.5-sonnet", api_key: str = None
):
