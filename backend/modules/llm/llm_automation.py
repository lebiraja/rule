...
from .provider_router import PROVIDER_REGISTRY, get_provider
...
class LLMAutomation:
    ...
    def _validate_and_fix_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if current_base_url != expected_base_url:
            print(
                f"[🔧 Config Fix] Updating base_url for {provider}: {current_base_url} -> {expected_base_url}"
            )
    ...
    def save_config(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            print(
                f"[✅ Config Saved] Provider: {config.get('provider')}, Model: {config.get('model')}"
            )
            return True
    ...
    def get_available_models(self, provider: str) -> List[str]:
        ...
        try:
            ...
            if hasattr(provider_class, "list_models"):
                ...
                print(
                    f"[⚠️ Warning] Provider {provider_name} does not have list_models method"
                )
                return []
        except Exception as e:
    ...
    def send_prompt_with_current_provider(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(provider, "send_prompt"):
                ...
                return {"success": False, "message": "Provider method not implemented"}
